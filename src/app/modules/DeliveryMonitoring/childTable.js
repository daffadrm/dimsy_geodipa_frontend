...
class ChildTable extends React.Component {
    ...
    changesRadio(indexRow, indexCell, event){
        ...
        this.setState({dataRow}, ()=>{
            this.props.callBackAddData(dataRow, indexRow, this.state.addChildSecond);
        })
    }
                                ...
                                <td className="align-middle td-6 text-center">
                                    {
                                        ...
                                        :
                                        <div>
                                            <i className="far fa-edit m-1 text-success pointer" onClick={() => {this.props.callBackEditData(indexRow, index);}}></i>
                                            <i className="far fa-trash-alt m-1 text-danger pointer" onClick={() => {this.props.callBackDeleteData(indexRow, index);}}></i>
                                        </div>
                                    }
                                </td>
                                <td className="align-middle td-8">Unduh</td>
                                <td className="align-middle td-8">{ item.upload_date_vendor ? window.moment(new Date(item.upload_date_vendor)).format("DD MMM YYYY") : "-" }</td>
                                <td className="align-middle td-8">Unduh</td>
                                <td className="align-middle td-8">{ item.upload_date_user ? window.moment(new Date(item.upload_date_user)).format("DD MMM YYYY") : "-" }</td>
                                <td className="align-middle td-6">-</td>
                                <td className="align-middle td-8 text-center">
                                <Form>
                                    <Form.Check inline label="Yes" type="radio" id={`inline-radio-1-` + index} value={true} checked={item.mandatory ? true : false } onChange={this.changesRadio.bind(this, indexRow, index)} name="mandatary" />
                                    <Form.Check inline label="no" type="radio" id={`inline-radio-2-` + index} value={false} checked={!item.mandatory ? true : false} onChange={this.changesRadio.bind(this, indexRow, index)} name="mandatary" />
                                </Form>
                                </td>
